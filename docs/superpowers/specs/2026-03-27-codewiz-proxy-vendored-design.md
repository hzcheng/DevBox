...
DevBox already supports native startup of the CodeWiz proxy flow through runtime configuration in `devbox/docker-compose.yml` and `devbox/start-services.sh`. However, the current integration still expects the proxy implementation to exist outside the DevBox repository at `/Projects/Repos/codewiz-proxy/proxy.py`.
...
DevBox startup will switch its default proxy path from the external repository location to the vendored location inside `/Projects/DevBox`. The `CODEWIZ_PROXY_SCRIPT` environment variable will remain supported as an override, so advanced users can still point DevBox at a different script if they need to test a custom proxy variant.
...
- `devbox/start-services.sh`
  Change the default `CODEWIZ_PROXY_SCRIPT` path from `/Projects/Repos/codewiz-proxy/proxy.py` to `/Projects/DevBox/devbox/codewiz-proxy/proxy.py`.
- `devbox/Dockerfile.x86_64`
  Copy the vendored proxy script into the image.
...
- `/Projects/DevBox/devbox/codewiz-proxy/proxy.py`

This keeps the runtime behavior aligned with the DevBox repository layout visible inside the container.
...
The mounted repository path `..:/Projects/DevBox` already makes the DevBox checkout available in the container during normal Compose runs. Even so, copying the proxy into the image is still desirable because:

- it guarantees the proxy exists for image-based usage patterns
- it keeps the image self-contained
- it reduces dependence on host-side path conventions

### Runtime Override Compatibility

`CODEWIZ_PROXY_SCRIPT` will remain a supported override. This preserves flexibility for testing alternate proxy implementations while making the vendored in-repo version the default.
...
2. Change DevBox runtime defaults to use the vendored path.
3. Ensure both Dockerfiles package the vendored script.
...
- confirm the proxy starts from the vendored in-repo path
...
- optionally override `CODEWIZ_PROXY_SCRIPT` to a custom path and confirm the override still works
...
- Hidden path assumptions in startup logic
  Mitigation: update only the default path and keep the environment-variable override.
