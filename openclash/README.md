...
## Included Mihomo Control Surface

This vendored deployment includes the built-in `mihomo` control surface:

- `external-controller` API exposed on `OPENCLASH_CONTROLLER_PORT`
- baked `metacubexd` web UI served by `mihomo`
- runtime traffic, connection, and proxy-group state visible through the controller/UI

It does **not** include a separate observability stack such as Prometheus, Grafana, or a dedicated metrics exporter.

## Required `.env` Values

Set these in `.env` (see `.example.env` for defaults):
...
- `OPENCLASH_BUILD_MIHOMO_IMAGE`: Optional base image override for `mihomo`, useful when Docker Hub access is unstable.
...
From the DevBox repository root, use the proxy-aware wrapper so the root `.env` can decide whether OpenClash is enabled.

- `PROXY_PROVIDER=none`: do not start OpenClash
- `PROXY_PROVIDER=external`: keep using explicit `BUILD_PROXY` / `RUNTIME_PROXY`
- `PROXY_PROVIDER=openclash`: start the vendored `openclash/` service and auto-route DevBox build/runtime through it
...
If you want to run the OpenClash subtree directly for isolated debugging, you can still use:

```bash
cd /Projects/DevBox/openclash
cp .example.env .env
docker compose --profile openclash --env-file .env config
docker compose --profile openclash --env-file .env build
docker compose --profile openclash --env-file .env up -d --remove-orphans
```
...
```bash
cd /Projects/DevBox/openclash
docker compose --profile openclash --env-file .env restart
```
...
cd /Projects/DevBox/openclash
docker compose --profile openclash --env-file .env build
docker compose --profile openclash --env-file .env up -d --force-recreate --remove-orphans
```
...
```bash
cd /Projects/DevBox/openclash
docker compose --profile openclash --env-file .env stop
docker compose --profile openclash --env-file .env start
```
...
- `.example.env`: deployment contract (copy to `.env` for local use)
